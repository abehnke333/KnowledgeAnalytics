...
# define probability of keeping a node during dropout = 1 at test time and .75 during training
...
with tf.variable_scope('layer_1'):
    ...
    biases = tf.get_variable(name="biases1", initializer=tf.ones([layer1_nodes])/10)
    layer_1_pre_dropout = tf.nn.relu(tf.matmul(X, weights) + biases)
    layer_1_output = tf.nn.dropout(layer_1_pre_dropout, pkeep) # keep 75 percent of the nodes   
...
with tf.variable_scope('layer_2'):
    ...
    # dont initialize bias to 0 because of relu (dont know why)
    biases = tf.get_variable(name='biases2', initializer=tf.ones(layer2_nodes)/10)
    layer_2_pre_dropout = tf.nn.relu(tf.matmul(layer_1_output, weights) + biases)
    layer_2_output = tf.nn.dropout(layer_2_pre_dropout, pkeep) # keep 75 percent of nodes
...
with tf.variable_scope('output'):
    # need to define as get_variabled because we need these variables to store
    # information as we go through our neural net
    weights = tf.get_variable(name='weights4',shape=[layer2_nodes,number_of_outputs],initializer=tf.contrib.layers.xavier_initializer())
    # dont initialize bias to 0 because of relu (dont know why)
...
#        for epoch in range(training_epochs):
        ...
        training_cost = 1
        while training_cost > .26664:
    
            # feed in training data and do a step of training
            session.run(optimizer, feed_dict={X: input_array, Y: output_array, pkeep: .75})
            
#            # use with for loop for training
#            if epoch % 100 == 0:
#                training_cost = session.run(cost, feed_dict={X: input_array, Y: output_array, pkeep: .75})
#                print(epoch, training_cost)
            training_cost = session.run(cost, feed_dict={X: input_array, Y: output_array, pkeep: .75})

        print("Training Is Complete")
        final_cost = session.run(cost, feed_dict={X: input_array, Y: output_array, pkeep: .75})
        print(final_cost)
...
#        output = session.run(prediction, feed_dict={X: val_x, pkeep: 1.0}) # run model on test input
...
#        cc_vals = classification_confidence.eval(feed_dict={X: val_x,pkeep:1.0},session=session)
#        
...
#                if abs(tpr[i] - (1-fpr[i])) < closest_to_zero:
#                    closest_to_zero = abs(tpr[i] - (1-fpr[i]))
#                    print('ctz: ', closest_to_zero)
#                    print('fpr: ', fpr[i])
#                    print('tpr: ', tpr[i])
#                    print('1-fpr: ', 1-fpr[i])
...
##################### Test Data ###################################################
        ...
        output = session.run(prediction, feed_dict={X: test_x, pkeep: 1.0}) # run model on test input
        classification_confidence_test = tf.sigmoid(output)

        # probabilities of being class 1
        cc_test = classification_confidence_test.eval(feed_dict={X: test_x, pkeep: 1.0},session=session)
        ...
        np.savetxt('tf_output_avg.csv',output_class,fmt='%s')     